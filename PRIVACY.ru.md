...
1. **Содержимое по умолчанию остаётся на устройстве.** Тексты записей, фотографии, оценки настроения, теги и эмбеддинги обрабатываются локально с использованием фреймворков Apple on-device. Бесплатные пользователи получают AI-функции (Ask Mira, еженедельные рефлексии) через Apple Foundation Models, которые работают полностью на iPhone — текст не покидает устройство.
...
### 5.3. API-ключи, предоставленные пользователем *(legacy, больше не предлагается)*

Ранние сборки Mira позволяли пользователям предоставлять собственные API-ключи Anthropic / OpenAI / OpenRouter в режиме «Bring Your Own Key». В выпущенной версии Mira **не запрашивает и не принимает** пользовательские API-ключи — Pro-AI доставляется исключительно через хостинг-прокси Mira (Раздел 5.7). Любые остаточные ключи в iOS Keychain от прежних сборок можно удалить в **Настройки → Интеллект**; мы их не передаём и не обрабатываем.

### 5.3a. *(historical)* API-ключи, предоставленные пользователем (опциональная функция)
...
- `productId` — какой план Mira Pro приобретён (`com.veilbytesoft.Mira.pro.monthly` или `.yearly`).
...
Когда у вас активна подписка Mira Pro и вы используете Pro AI-функцию, Cloudflare Worker Mira проксирует ваш промпт и контекст дневника на `api.anthropic.com` Anthropic. Anthropic выступает **обработчиком (processor)** в интересах Разработчика согласно своим Commercial Terms of Service. Согласно условиям Anthropic, **входные и выходные данные API не используются для обучения моделей Anthropic**. Подписан Data Processing Addendum (DPA) Anthropic, покрывающий субъектов EU/UK GDPR.
...
### 8.5. OpenAI, L.L.C., OpenRouter, Hugging Face — *не используются*

Ранние сборки предлагали альтернативных AI-провайдеров. Выпущенная версия не использует OpenAI и OpenRouter. К Hugging Face Hub приложение может обращаться только при загрузке файла on-device модели (содержимое дневника не передаётся, только HTTPS-запросы на загрузку файла).

Политика Hugging Face: <https://huggingface.co/privacy>.
...
| Legacy API-ключи (если остались от ранних сборок) | До момента удаления пользователем | Настройки → Intelligence (legacy-поля) |
