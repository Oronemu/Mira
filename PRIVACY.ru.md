...
- сторонние сервисы, к которым вы обращаетесь через Mira по собственному выбору (Anthropic, OpenAI, OpenRouter, Apple iCloud/CloudKit, Google Firebase, Hugging Face) — обработка данных такими сервисами регулируется их собственными политиками, ссылки на которые приведены в **Разделе 8**;
...
1. **У Разработчика нет собственного бэкенда.** Мы не эксплуатируем сервер, который принимал бы, хранил или обрабатывал содержимое ваших дневников. У нас нет технической возможности читать ваши записи.
2. **Содержимое по умолчанию остаётся на устройстве.** Тексты записей, фотографии, оценки настроения, теги, эмбеддинги и AI-промпты обрабатываются локально с использованием фреймворков Apple on-device.
3. **Любая передача данных за пределы устройства — только по явному согласию пользователя.** Облачные функции (резервная копия в iCloud, удалённый AI, аналитика, крашлитика) либо отключены по умолчанию, либо требуют прямого действия пользователя.
4. **Минимизация.** Мы обрабатываем только те данные, которые строго необходимы для активированной пользователем функции.
...
6. **Контроль пользователя.** Вы можете в любое время отключить телеметрию, отключить синхронизацию, удалить записи и удалить API-ключи — в разделе «Настройки».
...
- Векторные эмбеддинги ваших записей (512-мерные векторы с плавающей точкой, формируемые на устройстве фреймворком Apple NaturalLanguage) — используются для семантического поиска.
...
### 5.3. API-ключи, предоставленные пользователем (опциональная функция)

При активации функции «Remote AI» вы можете ввести собственный API-ключ сервиса Anthropic, OpenAI или OpenRouter.

- API-ключ хранится **исключительно** в iOS Keychain с атрибутом доступа `kSecAttrAccessibleWhenUnlockedThisDeviceOnly` — то есть привязан к конкретному устройству и **не** синхронизируется в iCloud.
- Ключ никогда не передаётся Разработчику. Приложение передаёт его **напрямую** выбранному AI-провайдеру по TLS.

**Правовые основания:** согласие пользователя; исполнение договора.
...
### 5.7. Данные Remote AI (опционально, по явному включению)

Если вы активировали Remote AI и инициируете запрос (вопрос в «Ask Mira», генерацию рефлексии и т. п.):

- Приложение отправляет **напрямую с вашего устройства** выбранному провайдеру по TLS промпт, состоящий из:
  - системных инструкций;
  - вашего текущего вопроса или запроса на генерацию;
  - контекста, извлечённого из вашего дневника (выбранный алгоритмом RAG поднабор текста записи, настроения и тегов), необходимого для ответа.
- Приложение прикладывает к запросу предоставленный вами API-ключ, так что биллинг идёт по **вашему** счёту у провайдера.

Разработчик **не** получает, **не** проксирует и **не** логирует эти данные. AI-провайдер выступает самостоятельным оператором и обрабатывает данные согласно собственной политике. См. Раздел 8.
...
- финансовую или платёжную информацию (в текущей версии Приложение не содержит встроенных покупок);
...
| 6.2 | Работа on-device AI-функций (эмбеддинги, семантический поиск, рефлексии, Ask Mira) на базе Apple Foundation Models | 5.1 | Согласие; Art. 9(2)(a) для настроения |
...
| 6.5 | Опциональный Remote AI при вашем собственном API-ключе | 5.3, 5.7 | Согласие; исполнение договора |
...
- **Генеративный AI на устройстве (Apple Foundation Models)** — формирование рефлексий и ответов. Результат отображается исключительно вам.
...
- **Apple Foundation Models** и фреймворк **NaturalLanguage:** работают на устройстве, данные устройство не покидают.
...
### 8.3. Anthropic PBC (Claude) — опционально

При выборе Anthropic в качестве Remote-AI-провайдера и вводе собственного API-ключа Приложение направляет промпты и выбранный контекст дневника напрямую на `api.anthropic.com`.

Политика: <https://www.anthropic.com/legal/privacy>.

### 8.4. OpenAI, L.L.C. — опционально

При выборе OpenAI и вводе собственного API-ключа Приложение направляет промпты напрямую в API OpenAI.

Политика: <https://openai.com/policies/privacy-policy>.

### 8.5. OpenRouter (OpenRouter, Inc.) — опционально

При выборе OpenRouter и вводе собственного API-ключа Приложение направляет промпты напрямую в API OpenRouter.

Политика: <https://openrouter.ai/privacy>.

### 8.6. Hugging Face, Inc. — опционально

При загрузке весов on-device моделей AI Приложение обращается к Hugging Face Hub по HTTPS для получения файлов модели. Содержимое дневника не передаётся — только запросы на загрузку файлов.

Политика: <https://huggingface.co/privacy>.

### 8.7. Иных получателей нет
...
| Anthropic | США | SCC в рамках DPA Anthropic |
| OpenAI | США | SCC в рамках DPA OpenAI |
| OpenRouter | США | SCC по условиям OpenRouter |
| Hugging Face | США / Франция | SCC / ЕС-инстанс для европейского региона |
...
| API-ключи | До момента удаления пользователем | Настройки → Intelligence → удалить ключ |
...
- **Хранение в Keychain** чувствительных материалов (API-ключи, ключи шифрования) с device-only или synchronisable атрибутами — в зависимости от назначения.
